...
export class App {
    ...
    constructor(public port: number) {
        ...
        this.serverData = JSON.parse(fs.readFileSync('./html/servers.json', {encoding: "utf-8"}));
    ...
    private setupRoutes() {
        ...
        this.app.get("/servers", (req: express.Request, res: express.Response) => {
            const address = req.ip as string;
    ...
    private onRegister(req: express.Request, res: express.Response) {
        ...
        if (this.accounts[login] !== undefined) return res.status(400).send("That login taken");
    ...
    private onLogin(req: express.Request, res: express.Response) {
        const {login, password} = req.body;
        ...
        if (account === undefined) return res.status(400).send("That login doesn't exist");
        if (account.password !== password) return res.status(400).send("Wrong password");

        fs.writeFileSync("./JSON/accounts.json", JSON.stringify(this.accounts));

        if (account.kit) account.kit = Math.max((account.kitStamp - Date.now()), 0);

        res.send(this.accounts[login]);
    }

    public onGetBreadAndScore(req: express.Request, res: express.Response) {
        const {login} = req.query;

        const account = this.accounts[login as any];
        if (!account) return res.status(429).send();

        res.send(JSON.stringify({s: account.seasons[0].score, b: account.bread}))
    }

    public onGetLeaderboard(req: express.Request, res: express.Response) {
        const {mode, sort, season, range} = req.query;

        if (!mode || !sort) return res.status(429).send();
        ...
        const lb = leaderboard.sort((a: any, b: any) => b[index] - a[index]);
        res.send(lb);
    }

    private onBuyKit(req: express.Request, res: express.Response) {
    ...
    private onBuySpin(req: express.Request, res: express.Response) {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            let type = (
                spinId < 3 ? SkinType.SKIN : Math.ceil(Math.random() * 4)
            );
            ...
            const skins = COSMETICS[SkinType[type]].filter((skin: any) => skin.rarity === Rarity.WOOD + offset);
    ...
    private updatePlayerCount(req: express.Request, res: express.Response) {
        const {c, d, p} = req.body;

        const server = this.serverData.find((s: any) => s.i === d);
        if (!Number.isInteger(c) || p !== Settings.password || !server) return;

        server.nu = c;
    }

    private updateAccountData(req: express.Request, res: express.Response) {
        const {l, s, k, t, p, e} = req.body;
        const account = this.accounts[l];


        if (!account || p !== Settings.password) return;
    ...
    private run() {
        this.app.get("/js/client.min.js", (req: express.Request, res: express.Response) => {
